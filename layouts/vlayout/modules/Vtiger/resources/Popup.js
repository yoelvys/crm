...
jQuery.Class("Vtiger_Popup_Js",{
	...
	show : function(urlOrParams, cb, windowName, eventName, onLoadCb){
        ...
        showCustom : function(urlOrParams, cb, windowName, eventName, onLoadCb){
		if(typeof urlOrParams == 'undefined'){
			urlOrParams = {};
		}
		if (typeof urlOrParams == 'object' && (typeof urlOrParams['view'] == "undefined")) {
			urlOrParams['view'] = 'PopupCustom';
		}
		...
		var popupWinRef =  window.open(url, windowName ,"width=" + width + ",height=" + height + ",menubar=0,toolbar=0,directories=0,resizable=0,scrollbars=1,top=" + top + ",left=" + left + "");
		if (typeof this.destroy == 'function') {
			// To remove form elements that have created earlier
			this.destroy();
		}
	...
	getListViewEntries: function(e){
		...
		if(typeof dataUrl != 'undefined'){
		    ...
		    var recordName = row.data('name');
                    var recordCode = row.data('code');
                    var recordInfo = row.data('info');
		    var response ={};
                    
		    response[id] = {'name' : recordName,'info' : recordInfo, 'code' : recordCode} ;
	...
	registerEventForSearch : function(){
        ...
        registerEventForSearchCustom : function() {
            ...
            $('#searchvalue').keyup(function() {
                ...
                if(search.length > 0) {
                    var i = 1;
                    $("#tableContent tr td.value").not(":Contains('" + search + "')").parent().hide();
                    $("#tableContent tr").each(function () {
	...
	registerEvents: function(){
		...
		this.registerEventForSearch();
                this.registerEventForSearchCustom();
